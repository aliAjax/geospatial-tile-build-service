...
func (s *Service) Start(ctx context.Context, id, version string, input []byte) (domain.Job, error) {
	// Build into an owned copy so concurrent callers sharing the same input slice
	// (or the HTTP handler handing us its json.RawMessage buffer) never see each
	// other's append, and the caller's backing array is left untouched. Mutating
	// the shared input in place produced both a data race and corrupted caller
	// buffers once two builds ran at once.
	compiled := make([]byte, len(input), len(input)+len("compiled-input"))
	copy(compiled, input)
	compiled = append(compiled, []byte("compiled-input")...)
	j := domain.Job{ID: id, VersionID: version, Status: domain.BuildRunning, InputHash: hash(compiled)}
	...
	s.jobs[id] = j
	s.mu.Unlock()
	if err := s.store.Put(ctx, "builds/"+id+"/input", compiled); err != nil {
		return s.fail(id, err)
	}
	out := sha256.Sum256(append([]byte("compiled:"), compiled...))
